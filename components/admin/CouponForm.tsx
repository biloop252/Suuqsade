...
export default function CouponForm({ coupon, onClose, onSave, isVendor = false, vendorId }: CouponFormProps) {
  ...
  useEffect(() => {
    if (coupon) {
      ...
      if (!coupon.is_global) {
        fetchExistingAssociations(coupon.id);
      }
    } else {
  ...
  useEffect(() => {
    if (!isVendor && formData.vendor_id !== coupon?.vendor_id) {
  ...
  const fetchExistingAssociations = async (couponId: string) => {
    try {
  ...
  const fetchData = async () => {
    try {
      ...
      const vendorIdForFiltering = isVendor ? vendorId : (formData.vendor_id || coupon?.vendor_id);
  ...
  const validateForm = () => {
    ...
    if (!formData.is_global && selectedProducts.length === 0 && selectedCategories.length === 0 && selectedBrands.length === 0) {
      newErrors.scope = 'Please select at least one product, category, or brand for specific coupons';
    }
            ...
            <div>
              ...
              <div className="flex">
                ...
                <button
                  ...
                  className="px-3 py-2 border border-l-0 border-gray-300 rounded-r-md bg-gray-50 hover:bg-gray-100 text-gray-600"
