...
-- This migration creates tables for managing promotional banners, sliders, popups, and other media
...
CREATE TABLE promotional_media (
  ...
  store_id UUID, -- For multi-store support (optional)
...
CREATE TABLE promotional_media_categories (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
CREATE INDEX idx_promotional_media_type ON promotional_media(media_type);
CREATE INDEX idx_promotional_media_position ON promotional_media(banner_position);
CREATE INDEX idx_promotional_media_active ON promotional_media(is_active);
CREATE INDEX idx_promotional_media_dates ON promotional_media(start_date, end_date);
CREATE INDEX idx_promotional_media_display_order ON promotional_media(display_order);
CREATE INDEX idx_promotional_media_created_by ON promotional_media(created_by);
CREATE INDEX idx_promotional_media_categories_media ON promotional_media_categories(promotional_media_id);
CREATE INDEX idx_promotional_media_categories_category ON promotional_media_categories(category_id);
...
COMMENT ON TABLE promotional_media IS 'Stores promotional banners, sliders, popups, and other media content';
COMMENT ON TABLE promotional_media_categories IS 'Junction table linking promotional media to categories';
COMMENT ON FUNCTION get_active_promotional_media(promotional_media_position, TEXT) IS 'Returns active promotional media for a specific position and language';
