-- Fix Vendor Relationships
-- This migration fixes the relationship issues between vendor_commissions and vendor_profiles

-- The issue is that vendor_commissions.vendor_id references profiles(id)
-- but vendor_profiles.id also references profiles(id)
-- We need to create a proper relationship or use a different approach
...
-- Option 1: Create a view that properly joins the tables
...
-- Option 2: Create a function to get vendor commissions with profile data
...
-- Create a similar function for vendor payouts
